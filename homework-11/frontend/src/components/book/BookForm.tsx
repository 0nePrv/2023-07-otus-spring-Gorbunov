...
export const BookForm: FC<FormProps<Book>> = ({obj: book, handleSubmit, handleCancel}) => {
  ...
  const schema = yup.object().shape({
    ...
    authorId: yup.string().required('Author is required').nonNullable(),
    genreId: yup.string().required('Genre is required').nonNullable()
  })
  ...
  return (
      <form className={'form-container'}
        ...
        <SelectComponent<Author>
            title={'Author'}
            state={watch('authorId') ?? ''}
            callback={(id: string) => setValue('authorId', id)}
        ...
        <SelectComponent<Genre>
            title={'Genre'}
            state={watch('genreId') ?? ''}
            callback={(id: string) => setValue('genreId', id)}
