...
import static ru.otus.homework.security.SecurityTestUtils.generateResourceActionPairs;
...
public class SecurityTest {
  ...
  @DisplayName("should provide anonymous access to public resources")
  @ParameterizedTest
  ...
  @DisplayName("should not provide anonymous access to secured resources")
  @ParameterizedTest
  @MethodSource("getAuthenticationTestParams")
  void shouldNotProvideAnonymousAccess(Pair<String, HttpMethod> pair) throws Exception {
    mockMvc.perform(request(pair.getSecond(), pair.getFirst()).with(anonymous()))
        .andExpect(unauthenticated());
  }

  @DisplayName("should check endpoint access to target user")
  @ParameterizedTest
  @MethodSource("getAuthorizationTestParams")
  void shouldCheckEndpointAccessToTargetUser(TestEntity entity) throws Exception {
    mockMvc.perform(request(entity.getMethod(), entity.getUri()).with(csrf())
        .with(user(entity.getUser())))
        .andExpect(authenticated())
        .andExpect(getExpectedResult(entity));
  }
  ...
  private static Stream<TestEntity> getAuthorizationTestParams() {
  ...
  private static Stream<Pair<String, HttpMethod>> getAuthenticationTestParams() {
    return generateResourceActionPairs();
  }
}